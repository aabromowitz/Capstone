# I was having issues with the temp data being too similar to a linear combination of vpmax and tdew.
# So try again, but increase that number.
# Also, add the logic to make it so vpmax has a max value, since that was causing issues too.

numSims = 50 
varFactor = 0.5 # less variance
temp_var = 0.01 
temp_phi = 0.999
library(tswge)
...
maxsPerSplit = numeric(numSplits)
for (split in 1:numSplits){
  t = ((split-1)*numPerSplit+1):(split*numPerSplit)
  max = max(weather$VPmax..mbar.[t])
  maxsPerSplit[split] = max
}
t=1:366
numDays = 366
...
for (seed in 1:numSims){
  ...
  for (split in 1:numSplits){
    ...
    min_threshold = min*minMultVPMax
    indices_to_change = t[y_gen_vpmax_noise[t] < min_threshold]
    y_gen_vpmax_noise[indices_to_change] = min_threshold
    max = maxsPerSplit[split]
    max_threshold = max*maxMultVPMax
    indices_to_change = t[y_gen_vpmax_noise[t] > max_threshold]
    y_gen_vpmax_noise[indices_to_change] = max_threshold
  }
  ...
  # Temps
  varTemp = temp_var * varFactor
  # varLin = rnorm(n=length(y_gen_TLin_adj[,sim]),mean=0,sd=sqrt(varTemp))
  varLin = gen.arma.wge(n=length(y_gen_TLin_adj[,sim]),phi= temp_phi,mu=0,vara=varTemp,plot=FALSE)
  # varNonlin = rnorm(n=length(y_gen_TNonlin_adj[,sim]),mean=0,sd=sqrt(varTemp))
  varNonlin = gen.arma.wge(n=length(y_gen_TNonlin_adj[,sim]),phi= temp_phi,mu=0,vara=varTemp,plot=FALSE)
...
# Save off CSVs
library(readr)
write_csv(y_gen_vpmax_adj, "C:/Users/aabro/OneDrive/Desktop/SMU Program/Capstone/Sims/vpmax_1_to_50_varTemp001varFac05.csv")
write_csv(y_gen_tdew_adj, "C:/Users/aabro/OneDrive/Desktop/SMU Program/Capstone/Sims/tdew_1_to_50_varTemp001varFac05.csv")
write_csv(y_gen_TLin_adj, "C:/Users/aabro/OneDrive/Desktop/SMU Program/Capstone/Sims/temp_1_to_50_varTemp001varFac05.csv")
write_csv(y_gen_TNonlin_adj, "C:/Users/aabro/OneDrive/Desktop/SMU Program/Capstone/Sims/tempNonLin_1_to_50_varTemp001varFac05.csv")

################################################################################

# Phi for temp data
est.ar.wge(weather$T..degC.,p=1) # avar = 0.06424383, phi = 0.9994197
gen.arma.wge(n=100,phi= 0.999,mu=0,vara=0.01,plot=TRUE)
plot(rnorm(n=100,mean=0,sd=sqrt(0.1)))