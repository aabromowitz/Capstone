# Shoot, TFT wasn't training on our simulated data.
# I wonder if it will train properly on the signal, if I remove the noise?
...
y_gen_vpmax_true = y_gen_vpmax_true[1:(6*24*(dayStop-dayStart+1))]
...
# Create "true" temp
y_gen_TLin_true = -2.274795  + y_gen_vpmax_true*0.815397 + y_gen_tdew_true*0.236779

# Plot VPMax for the full year
t = 1:(6*24*366)
t = t/(6*24)
len = length(weather$date)
plot(x=t[1:len],y=y_gen_vpmax_true[1:len], type = "p", col = "blue", pch = 16, cex = 0.5,
     xlab = "Days since 1/1/2020", ylab = "Vapor Pressure (mbar)", 
     main = "Maximum Vapor Pressure Signal - Full Year")

# Plot VPMAX for first week
startDay = 0
startNum = startDay*6*24
len = 6*24*7
plot(x=t[startNum:(startNum+len)],y=y_gen_vpmax_true[startNum:(startNum+len)], 
     type = "p", col = "blue", pch = 16, cex = 0.8,
     xlab = "Days since 1/1/2020", ylab = "Vapor Pressure (mbar)", 
     main = "Maximum Vapor Pressure Signal - Week in Winter")

# Plot VPMAX for middle week
startDay = 180
startNum = startDay*6*24
len = 6*24*7
plot(x=t[startNum:(startNum+len)],y=y_gen_vpmax_true[startNum:(startNum+len)], 
     type = "p", col = "blue", pch = 16, cex = 0.8,
     xlab = "Days since 1/1/2020", ylab = "Vapor Pressure (mbar)", 
     main = "Maximum Vapor Pressure Signal - Week in Summer")

# Three plots for TDEW
t = 1:(6*24*366)
t = t/(6*24)
len = length(weather$date)
plot(x=t[1:len],y=y_gen_tdew_true[1:len], type = "p", col = "blue", pch = 16, cex = 0.5,
     xlab = "Days since 1/1/2020", ylab = "Temperature (C)", 
     main = "Dew Temperature Signal - Full Year")
startDay = 0
startNum = startDay*6*24
len = 6*24*7
plot(x=t[startNum:(startNum+len)],y=y_gen_tdew_true[startNum:(startNum+len)], 
     type = "p", col = "blue", pch = 16, cex = 0.8,
     xlab = "Days since 1/1/2020", ylab = "Temperature (C)", 
     main = "Dew Temperature Signal - Week in Winter")
startDay = 180
startNum = startDay*6*24
len = 6*24*7
plot(x=t[startNum:(startNum+len)],y=y_gen_tdew_true[startNum:(startNum+len)], 
     type = "p", col = "blue", pch = 16, cex = 0.8,
     xlab = "Days since 1/1/2020", ylab = "Temperature (C)", 
     main = "Dew Temperature Signal - Week in Summer")

# Three plots for temp
t = 1:(6*24*366)
t = t/(6*24)
len = length(weather$date)
plot(x=t[1:len],y=y_gen_TLin_true[1:len], type = "p", col = "blue", pch = 16, cex = 0.5,
     xlab = "Days since 1/1/2020", ylab = "Temperature (C)", 
     main = "Air Temperature Signal - Full Year")
startDay = 0
startNum = startDay*6*24
len = 6*24*7
plot(x=t[startNum:(startNum+len)],y=y_gen_TLin_true[startNum:(startNum+len)], 
     type = "p", col = "blue", pch = 16, cex = 0.8,
     xlab = "Days since 1/1/2020", ylab = "Temperature (C)", 
     main = "Air Temperature Signal - Week in Winter")
startDay = 180
startNum = startDay*6*24
len = 6*24*7
plot(x=t[startNum:(startNum+len)],y=y_gen_TLin_true[startNum:(startNum+len)], 
     type = "p", col = "blue", pch = 16, cex = 0.8,
     xlab = "Days since 1/1/2020", ylab = "Temperature (C)", 
     main = "Air Temperature Signal - Week in Summer")

# Create a CSV to see how TFT trains with it
y_true <- data.frame(
  date = weather['date'],
  temp = y_gen_TLin_true,
  vpmax = y_gen_vpmax_true,
  tdew = y_gen_tdew_true
)
column_names <- c("date", "T (degC)", "VPmax (mbar)", "Tdew (degC)")
colnames(y_true) <- column_names
library(readr)
write_csv(y_true, "C:/Users/aabro/OneDrive/Desktop/SMU Program/Capstone/Sims/Signal.csv")

################################################################################

# Listing all the vars from the data generation part
# VPMAX:
#   b0var = 4
#   ampVar = 0.3 (log transformation)
#   varB = -4 (log transformation)
#   varAmp = abs(-1.63190) (log transformation)
#   varVar = 0.6 (log transformation)
#   varSin = exp(varAmp * cos(2 * pi * f * t + varPsi) + varB + gen.arma.wge(n=366,phi=varPhi,theta = 0, vara = varVar, sn = 0, plot=FALSE))
# TDEW:
#   b0sin = b0sin_true + gen.arma.wge(n=366, phi=0.7, theta=0, sn=0, vara =5.48, plot = FALSE), so var = 5.48
#   Amp_avar = 0.9459506
#   varA = -0.89346 (log transformation)
#   varB = -3.75244 (log transformation)
#   Var_avar = 0.4903821 (log transformation)
...
# What happens when I introduce a variance reduction factor?
numSims = 50 # Start with 20, to get plots
varFactor = 0.5 # start with 0.5
library(tswge)
file_path = "https://raw.githubusercontent.com/aabromowitz/Capstone/refs/heads/main/weather_interpolation.csv"
weather <- read.csv(file_path, header = TRUE)
file_path = "https://raw.githubusercontent.com/aabromowitz/Capstone/refs/heads/main/vectors_VPMax.csv"
...
for (seed in 1:numSims){
  ...
  varSin = exp(varAmp * cos(2 * pi * f * t + varPsi) + varB + gen.arma.wge(n=366,phi=varPhi,theta = 0, vara = varVar, sn = 0, plot=FALSE)) * varFactor
  ...
  t = 1:366
  b0sin_true = b0A * cos(2 * pi * f * t + b0Psi) + b0B
  b0sin = b0sin_true + gen.arma.wge(n=366, phi=0.7, theta=0, sn=0, vara =b0_avar, plot = FALSE)
  Asin_true = AA * cos(2 * pi * f * t + APsi) + AB
  Asin = Asin_true + gen.arma.wge(n=366, phi=Amp_phi, theta=0, sn=0, vara =Amp_avar, plot = FALSE)
  logVar_sin_true = varA * cos(2 * pi * f * t + varPsi) + varB
  logVar_sin = logVar_sin_true + gen.arma.wge(n=366, phi=Var_phi, theta=0, sn=0, vara =Var_avar, plot = FALSE)
  var_sin = exp(logVar_sin) * varFactor
  ...
  # Temps
  varTemp = 0.1 * varFactor
  varLin = rnorm(n=length(y_gen_TLin_adj[sim]),mean=0,sd=sqrt(varTemp))
  varNonlin = rnorm(n=length(y_gen_TNonlin_adj[sim]),mean=0,sd=sqrt(varTemp))
  y_gen_TLin_adj[,sim] = -2.274795  + y_gen_vpmax_adj[,sim]*0.815397 + y_gen_tdew_adj[,sim]*0.236779 + varLin
  y_gen_TNonlin_adj[,sim] = 44.06+243.5*log(y_gen_vpmax_adj[,sim]/100)/(17.62-log(y_gen_vpmax_adj[,sim]/100))*1.326+y_gen_vpmax_adj[,sim]*2.102e-02-y_gen_tdew_adj[,sim]*1.000e-03+y_gen_vpmax_adj[,sim]*y_gen_tdew_adj[,sim]*6.250e-05 + varNonlin
}
...
# Save off sample realization in CSV
y_var_fac <- data.frame(
  date = weather['date'],
  temp = y_gen_TLin_adj[,1],
  vpmax = y_gen_vpmax_adj[,1],
  tdew = y_gen_tdew_adj[,1]
)
column_names <- c("date", "T (degC)", "VPmax (mbar)", "Tdew (degC)")
colnames(y_var_fac) <- column_names
library(readr)
write_csv(y_var_fac, "C:/Users/aabro/OneDrive/Desktop/SMU Program/Capstone/Sims/VarFac1_Single_Attempt2.csv")

################################################################################

library(readr)
write_csv(y_gen_vpmax_adj, "C:/Users/aabro/OneDrive/Desktop/SMU Program/Capstone/Sims/vpmax_1_to_50_var05.csv")
write_csv(y_gen_tdew_adj, "C:/Users/aabro/OneDrive/Desktop/SMU Program/Capstone/Sims/tdew_1_to_50_var05.csv")
write_csv(y_gen_TLin_adj, "C:/Users/aabro/OneDrive/Desktop/SMU Program/Capstone/Sims/temp_1_to_50_var05.csv")